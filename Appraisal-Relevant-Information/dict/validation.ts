...
const ApiRelevantDataDtoKeys: string[] = ['siNo', 'details']

function modelApiRelevantDataDtoFromJsonContent(name: string, value: any, knownKeys: Record<string, boolean> = {}): Api.RelevantDataDto {
	if (typeof value !== 'object' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected object got ${typeof value}`
	}

	ApiRelevantDataDtoKeys.forEach(k => knownKeys[k] = true)

	const result: Api.RelevantDataDto = {
...
function modelApiRelevantDataDtoToJsonContent(name: string, value: Api.RelevantDataDto, knownKeys: Record<string, boolean> = {}): ToJson<Api.RelevantDataDto> {
	if (typeof value !== 'object' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected object got ${typeof value}`
	}
	...
	const result: ToJson<Api.RelevantDataDto> = {
...
export function modelApiRelevantDataDtoFromJson(name: string, value: any): Api.RelevantDataDto {
	const knownKeys: Record<string, boolean> = {}
	const result: Api.RelevantDataDto = modelApiRelevantDataDtoFromJsonContent(name, value, knownKeys)
	...
	for (const key of Object.keys(value)) {
		if (!knownKeys[key]) {
			// throw `Unexpected key: ${key}`
			console.warn(`Unexpected key in Api.RelevantDataDto: ${key}`)
		}
	}
...
export function modelApiRelevantDataDtoToJson(name: string, value: Api.RelevantDataDto): ToJson<Api.RelevantDataDto> {
	const knownKeys: Record<string, boolean> = {}
	const result: ToJson<Api.RelevantDataDto> = modelApiRelevantDataDtoToJsonContent(name, value, knownKeys)
...
const ApiRelevantDataPagedResultDataKeys: string[] = ['totalCount', 'items']

function modelApiRelevantDataPagedResultDataFromJsonContent(name: string, value: any, knownKeys: Record<string, boolean> = {}): Api.RelevantDataPagedResultData {
	if (typeof value !== 'object' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected object got ${typeof value}`
	}

	ApiRelevantDataPagedResultDataKeys.forEach(k => knownKeys[k] = true)

	const result: Api.RelevantDataPagedResultData = {
		'totalCount': allowUndefined(integerFromJson)(`${name}.totalCount`, value['totalCount']),
		'items': allowUndefined(arrayFromJson(modelApiRelevantDataDtoFromJson))(`${name}.items`, value['items']),
	}

	return result
}

function modelApiRelevantDataPagedResultDataToJsonContent(name: string, value: Api.RelevantDataPagedResultData, knownKeys: Record<string, boolean> = {}): ToJson<Api.RelevantDataPagedResultData> {
	if (typeof value !== 'object' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected object got ${typeof value}`
	}
	...
	const result: ToJson<Api.RelevantDataPagedResultData> = {
		'totalCount': allowUndefined(integerToJson)(`${name}.totalCount`, value['totalCount']),
		'items': allowUndefined(arrayToJson(modelApiRelevantDataDtoToJson))(`${name}.items`, value['items']),
	}

	return result
}

export function modelApiRelevantDataPagedResultDataFromJson(name: string, value: any): Api.RelevantDataPagedResultData {
	const knownKeys: Record<string, boolean> = {}
	const result: Api.RelevantDataPagedResultData = modelApiRelevantDataPagedResultDataFromJsonContent(name, value, knownKeys)
	...
	for (const key of Object.keys(value)) {
		if (!knownKeys[key]) {
			// throw `Unexpected key: ${key}`
			console.warn(`Unexpected key in Api.RelevantDataPagedResultData: ${key}`)
		}
	}
...
export function modelApiRelevantDataPagedResultDataToJson(name: string, value: Api.RelevantDataPagedResultData): ToJson<Api.RelevantDataPagedResultData> {
	const knownKeys: Record<string, boolean> = {}
	const result: ToJson<Api.RelevantDataPagedResultData> = modelApiRelevantDataPagedResultDataToJsonContent(name, value, knownKeys)
