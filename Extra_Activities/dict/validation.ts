...
const ApiExtraactivitiesDtoKeys: string[] = ['SINO', 'Activity', 'APIscore']

function modelApiExtraactivitiesDtoFromJsonContent(name: string, value: any, knownKeys: Record<string, boolean> = {}): Api.ExtraactivitiesDto {
	if (typeof value !== 'object' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected object got ${typeof value}`
	}

	ApiExtraactivitiesDtoKeys.forEach(k => knownKeys[k] = true)

	const result: Api.ExtraactivitiesDto = {
		...
		'APIscore': allowUndefined(stringFromJson)(`${name}.APIscore`, value['APIscore']),
	}

	return result
}

function modelApiExtraactivitiesDtoToJsonContent(name: string, value: Api.ExtraactivitiesDto, knownKeys: Record<string, boolean> = {}): ToJson<Api.ExtraactivitiesDto> {
	if (typeof value !== 'object' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected object got ${typeof value}`
	}
	...
	const result: ToJson<Api.ExtraactivitiesDto> = {
		'SINO': allowUndefined(stringToJson)(`${name}.SINO`, value['SINO']),
		'Activity': allowUndefined(stringToJson)(`${name}.Activity`, value['Activity']),
		'APIscore': allowUndefined(stringToJson)(`${name}.APIscore`, value['APIscore']),
	}

	return result
}

export function modelApiExtraactivitiesDtoFromJson(name: string, value: any): Api.ExtraactivitiesDto {
	const knownKeys: Record<string, boolean> = {}
	const result: Api.ExtraactivitiesDto = modelApiExtraactivitiesDtoFromJsonContent(name, value, knownKeys)
	...
	for (const key of Object.keys(value)) {
		if (!knownKeys[key]) {
			...
			console.warn(`Unexpected key in Api.ExtraactivitiesDto: ${key}`)
		}
	}
...
export function modelApiExtraactivitiesDtoToJson(name: string, value: Api.ExtraactivitiesDto): ToJson<Api.ExtraactivitiesDto> {
	const knownKeys: Record<string, boolean> = {}
	const result: ToJson<Api.ExtraactivitiesDto> = modelApiExtraactivitiesDtoToJsonContent(name, value, knownKeys)
...
const ApiExtraactivitiesPagedResultDtoKeys: string[] = ['totalCount', 'items']

function modelApiExtraactivitiesPagedResultDtoFromJsonContent(name: string, value: any, knownKeys: Record<string, boolean> = {}): Api.ExtraactivitiesPagedResultDto {
	if (typeof value !== 'object' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected object got ${typeof value}`
	}

	ApiExtraactivitiesPagedResultDtoKeys.forEach(k => knownKeys[k] = true)

	const result: Api.ExtraactivitiesPagedResultDto = {
		'totalCount': allowUndefined(integerFromJson)(`${name}.totalCount`, value['totalCount']),
		'items': allowUndefined(arrayFromJson(modelApiExtraactivitiesDtoFromJson))(`${name}.items`, value['items']),
	}

	return result
}

function modelApiExtraactivitiesPagedResultDtoToJsonContent(name: string, value: Api.ExtraactivitiesPagedResultDto, knownKeys: Record<string, boolean> = {}): ToJson<Api.ExtraactivitiesPagedResultDto> {
	if (typeof value !== 'object' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected object got ${typeof value}`
	}
	...
	const result: ToJson<Api.ExtraactivitiesPagedResultDto> = {
		'totalCount': allowUndefined(integerToJson)(`${name}.totalCount`, value['totalCount']),
		'items': allowUndefined(arrayToJson(modelApiExtraactivitiesDtoToJson))(`${name}.items`, value['items']),
	}

	return result
}

export function modelApiExtraactivitiesPagedResultDtoFromJson(name: string, value: any): Api.ExtraactivitiesPagedResultDto {
	const knownKeys: Record<string, boolean> = {}
	const result: Api.ExtraactivitiesPagedResultDto = modelApiExtraactivitiesPagedResultDtoFromJsonContent(name, value, knownKeys)
	...
	for (const key of Object.keys(value)) {
		if (!knownKeys[key]) {
			// throw `Unexpected key: ${key}`
			console.warn(`Unexpected key in Api.ExtraactivitiesPagedResultDto: ${key}`)
		}
	}
...
export function modelApiExtraactivitiesPagedResultDtoToJson(name: string, value: Api.ExtraactivitiesPagedResultDto): ToJson<Api.ExtraactivitiesPagedResultDto> {
	const knownKeys: Record<string, boolean> = {}
	const result: ToJson<Api.ExtraactivitiesPagedResultDto> = modelApiExtraactivitiesPagedResultDtoToJsonContent(name, value, knownKeys)
