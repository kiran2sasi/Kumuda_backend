...
const ApiTeachingDataKeys: string[] = ['siNo', 'course', 'level', 'modeOfTeaching', 'classesAllotted', 'classesConducted', 'facultyName']

function modelApiTeachingDataFromJsonContent(name: string, value: any, knownKeys: Record<string, boolean> = {}): Api.TeachingData {
	if (typeof value !== 'object' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected object got ${typeof value}`
	}

	ApiTeachingDataKeys.forEach(k => knownKeys[k] = true)

	const result: Api.TeachingData = {
		'siNo': allowUndefined(stringFromJson)(`${name}.siNo`, value['siNo']),
		'course': allowUndefined(stringFromJson)(`${name}.course`, value['course']),
		'level': allowUndefined(stringFromJson)(`${name}.level`, value['level']),
		'modeOfTeaching': allowUndefined(stringFromJson)(`${name}.modeOfTeaching`, value['modeOfTeaching']),
		'classesAllotted': allowUndefined(numberFromJson)(`${name}.classesAllotted`, value['classesAllotted']),
		'classesConducted': allowUndefined(numberFromJson)(`${name}.classesConducted`, value['classesConducted']),
		'facultyName': allowUndefined(stringFromJson)(`${name}.facultyName`, value['facultyName']),
	}

	return result
}

function modelApiTeachingDataToJsonContent(name: string, value: Api.TeachingData, knownKeys: Record<string, boolean> = {}): ToJson<Api.TeachingData> {
	if (typeof value !== 'object' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected object got ${typeof value}`
	}
	...
	const result: ToJson<Api.TeachingData> = {
		'siNo': allowUndefined(stringToJson)(`${name}.siNo`, value['siNo']),
		'course': allowUndefined(stringToJson)(`${name}.course`, value['course']),
		'level': allowUndefined(stringToJson)(`${name}.level`, value['level']),
		'modeOfTeaching': allowUndefined(stringToJson)(`${name}.modeOfTeaching`, value['modeOfTeaching']),
		'classesAllotted': allowUndefined(numberToJson)(`${name}.classesAllotted`, value['classesAllotted']),
		'classesConducted': allowUndefined(numberToJson)(`${name}.classesConducted`, value['classesConducted']),
		'facultyName': allowUndefined(stringToJson)(`${name}.facultyName`, value['facultyName']),
	}

	return result
}

export function modelApiTeachingDataFromJson(name: string, value: any): Api.TeachingData {
	const knownKeys: Record<string, boolean> = {}
	const result: Api.TeachingData = modelApiTeachingDataFromJsonContent(name, value, knownKeys)
	...
	for (const key of Object.keys(value)) {
		if (!knownKeys[key]) {
			// throw `Unexpected key: ${key}`
			console.warn(`Unexpected key in Api.TeachingData: ${key}`)
		}
	}
...
export function modelApiTeachingDataToJson(name: string, value: Api.TeachingData): ToJson<Api.TeachingData> {
	const knownKeys: Record<string, boolean> = {}
	const result: ToJson<Api.TeachingData> = modelApiTeachingDataToJsonContent(name, value, knownKeys)
...
const ApiTeachingPagedResultDataKeys: string[] = ['totalCount', 'items']

function modelApiTeachingPagedResultDataFromJsonContent(name: string, value: any, knownKeys: Record<string, boolean> = {}): Api.TeachingPagedResultData {
	if (typeof value !== 'object' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected object got ${typeof value}`
	}

	ApiTeachingPagedResultDataKeys.forEach(k => knownKeys[k] = true)

	const result: Api.TeachingPagedResultData = {
		'totalCount': allowUndefined(integerFromJson)(`${name}.totalCount`, value['totalCount']),
		'items': allowUndefined(arrayFromJson(modelApiTeachingDataFromJson))(`${name}.items`, value['items']),
	}

	return result
}

function modelApiTeachingPagedResultDataToJsonContent(name: string, value: Api.TeachingPagedResultData, knownKeys: Record<string, boolean> = {}): ToJson<Api.TeachingPagedResultData> {
	if (typeof value !== 'object' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected object got ${typeof value}`
	}
	...
	const result: ToJson<Api.TeachingPagedResultData> = {
		'totalCount': allowUndefined(integerToJson)(`${name}.totalCount`, value['totalCount']),
		'items': allowUndefined(arrayToJson(modelApiTeachingDataToJson))(`${name}.items`, value['items']),
	}

	return result
}

export function modelApiTeachingPagedResultDataFromJson(name: string, value: any): Api.TeachingPagedResultData {
	const knownKeys: Record<string, boolean> = {}
	const result: Api.TeachingPagedResultData = modelApiTeachingPagedResultDataFromJsonContent(name, value, knownKeys)
	...
	for (const key of Object.keys(value)) {
		if (!knownKeys[key]) {
			// throw `Unexpected key: ${key}`
			console.warn(`Unexpected key in Api.TeachingPagedResultData: ${key}`)
		}
	}
...
export function modelApiTeachingPagedResultDataToJson(name: string, value: Api.TeachingPagedResultData): ToJson<Api.TeachingPagedResultData> {
	const knownKeys: Record<string, boolean> = {}
	const result: ToJson<Api.TeachingPagedResultData> = modelApiTeachingPagedResultDataToJsonContent(name, value, knownKeys)
